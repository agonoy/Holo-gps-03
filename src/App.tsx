...
const SESSION_STORAGE_KEY = 'holo_gps_03_session_v1';
    ...
    >
      <div className="bg-panel/90 backdrop-blur border border-border px-3 py-2 rounded-lg shadow-xl flex items-center gap-3 transition-all">
        ...
        >
          ...
          {mapRotationMode === 'heading' ? 'Follow Heading' : 'North Up'}
        </button>
      </div>
    </motion.div>
  );
...
export default function App() {
  ...
  const [offlinePackState, setOfflinePackState] = useState<'idle' | 'downloading' | 'ready' | 'error'>('idle');
  const [offlinePackProgress, setOfflinePackProgress] = useState({ done: 0, total: 0 });
  const [offlinePackMessage, setOfflinePackMessage] = useState('Not downloaded');
  const [offlinePackZoomLabel, setOfflinePackZoomLabel] = useState('z11-z14');
  ...
  useEffect(() => {
    if (!('serviceWorker' in navigator)) {
      setOfflinePackMessage('Service worker unsupported');
      return;
    }
    ...
    const onServiceWorkerMessage = (event: MessageEvent) => {
      ...
      const zoomLabel =
        typeof data.minZoom === 'number' && typeof data.maxZoom === 'number'
          ? `z${data.minZoom}-z${data.maxZoom}`
          : offlinePackZoomLabel;

      if (data.type === 'OFFLINE_PACK_STARTED') {
        setOfflinePackState('downloading');
        setOfflinePackProgress({ done: 0, total: data.total ?? 0 });
        setOfflinePackZoomLabel(zoomLabel);
        setOfflinePackMessage(`Preparing Oahu offline map (${zoomLabel})...`);
      }

      if (data.type === 'OFFLINE_PACK_PROGRESS') {
        setOfflinePackState('downloading');
        setOfflinePackProgress({ done: data.done ?? 0, total: data.total ?? 0 });
        setOfflinePackZoomLabel(zoomLabel);
        setOfflinePackMessage(`Downloading Oahu offline tiles (${zoomLabel})...`);
      }

      if (data.type === 'OFFLINE_PACK_READY') {
        setOfflinePackState('ready');
        setOfflinePackProgress({ done: data.done ?? 0, total: data.total ?? 0 });
        setOfflinePackZoomLabel(zoomLabel);
        setOfflinePackMessage(`Oahu offline map ready (${zoomLabel})`);
      }

      if (data.type === 'OFFLINE_PACK_CLEARED') {
        setOfflinePackState('idle');
        setOfflinePackProgress({ done: 0, total: 0 });
        setOfflinePackMessage('Offline map cache cleared');
      }

      if (data.type === 'OFFLINE_PACK_ERROR') {
        setOfflinePackState('error');
        setOfflinePackZoomLabel(zoomLabel);
        setOfflinePackMessage(data.message || `Failed to cache offline tiles (${zoomLabel})`);
      }
    };
  ...
  const postToServiceWorker = async (message: Record<string, unknown>): Promise<boolean> => {
    if (!('serviceWorker' in navigator)) {
      return false;
    }
  ...
  const downloadOahuOfflinePack = async (minZoom: number, maxZoom: number) => {
    try {
      const zoomLabel = `z${minZoom}-z${maxZoom}`;
      setOfflinePackState('downloading');
      setOfflinePackProgress({ done: 0, total: 0 });
      setOfflinePackZoomLabel(zoomLabel);
      setOfflinePackMessage(`Starting offline download (${zoomLabel})...`);
      const sent = await postToServiceWorker({ type: 'DOWNLOAD_OAHU_PACK', minZoom, maxZoom });
      if (!sent) {
        setOfflinePackState('error');
        setOfflinePackMessage('Service worker not ready. Reload and try again.');
      }
    } catch (e) {
      setOfflinePackState('error');
      setOfflinePackMessage(`Offline pack failed: ${formatError(e)}`);
    }
  };

  const clearOahuOfflinePack = async () => {
    try {
      const sent = await postToServiceWorker({ type: 'CLEAR_OAHU_PACK' });
      if (!sent) {
        setOfflinePackState('error');
        setOfflinePackMessage('Service worker not ready. Reload and try again.');
      }
      ...
      setOfflinePackState('error');
      setOfflinePackMessage(`Unable to clear cache: ${formatError(e)}`);
    }
  };

  const exportAllLocalData = () => {
    const backup: AppBackupPayload = {
      version: 1,
      exportedAt: Date.now(),
      data: {
        vehicles,
    ...
    downloadFile(
      ...
      `holo-gps-03-backup-${datePart}.json`,
      'application/json',
    );
  };

  const importAllLocalData = async (event: ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      const parsed = JSON.parse(content) as Partial<AppBackupPayload> & {
        vehicles?: unknown;
        rides?: unknown;
        trails?: unknown;
      };

      const payloadData = parsed.data && typeof parsed.data === 'object'
        ? parsed.data
        : {
      ...
      if (!Array.isArray(payloadData.vehicles) || !Array.isArray(payloadData.rides) || !Array.isArray(payloadData.trails)) {
        throw new Error('Invalid backup format');
      }

      setVehicles(payloadData.vehicles as Vehicle[]);
      setRides(payloadData.rides as Ride[]);
      setTrails(payloadData.trails as Trail[]);
      setSelectedTrailId(null);
      setGpsError(null);
      setCurrentAddress('Imported local data');
    } catch (e) {
      setGpsError(`Import failed: ${formatError(e)}`);
    } finally {
      event.target.value = '';
    }
  };
      ...
      <header className="h-16 bg-panel border-b border-border flex items-center px-4 sm:px-6 justify-between shrink-0 z-[110]">
        <div className="flex items-center gap-3">
          ...
          <div className="flex items-center gap-2 font-extrabold text-base sm:text-xl tracking-tight">
            HOLO<span className="text-accent">GPS 03</span>
          </div>
        </div>
      ...
      <main className="flex flex-1 overflow-hidden relative">
        ...
        >
          ...
          <div className="sleek-card">
            <div className="sleek-label">User Profiles</div>
        ...
        <section className="flex-1 bg-[#0c111d] relative group/map">
                ...
                onToggle={async () => {
                  if (mapRotationMode === 'north-up') {
                    ...
                    if (!permissionGranted) {
                      setGpsError('Compass permission denied on this device/browser.');
                      return;
                    }
                    setGpsError(null);
                    setFollowMode(true);
          ...
          <div className="absolute top-20 right-6 z-[1000] pointer-events-none">
            <div className="bg-panel/90 backdrop-blur border border-border rounded-full h-11 w-11 flex items-center justify-center shadow-lg">
              <Compass size={16} className="text-text-dim" />
              <span className="absolute top-1.5 text-[9px] font-black text-red-500">N</span>
            </div>
          </div>
          ...
          <div className="absolute bottom-6 right-6 flex flex-col gap-2 z-[1000]">
            <button 
              ...
              title="Recenter and reset north-up"
            >
              <Target size={20} />
            </button>
            {selectedTrailId && (
        ...
        >
            ...
            <div className="bg-bg/40 p-4 rounded-lg border border-border/50 space-y-3">
              ...
              <div className="pt-3 border-t border-border/60 space-y-2">
                <div className="flex items-center justify-between text-[10px] font-bold uppercase">
                  <span className="text-text-dim">Offline Oahu Map</span>
                  <span className={`text-[9px] ${offlinePackState === 'ready' ? 'text-green-400' : offlinePackState === 'error' ? 'text-red-400' : 'text-text-dim'}`}>
                    {offlinePackState === 'ready' ? 'Ready' : offlinePackState === 'downloading' ? 'Caching' : offlinePackState === 'error' ? 'Error' : 'Idle'}
                  </span>
                </div>
                <p className="text-[9px] text-text-dim leading-relaxed">Choose a preset, download while online, then Oahu tiles will remain usable offline in this browser.</p>
                <p className="text-[9px] text-text-dim">Standard z11-z14: 1390 tiles. High detail z15-z16: 19182 tiles.</p>
                <p className="text-[9px] text-text-dim">{offlinePackMessage}</p>
                {offlinePackProgress.total > 0 && (
                  <p className="text-[9px] text-accent">{offlinePackProgress.done}/{offlinePackProgress.total} tiles ({offlinePackZoomLabel})</p>
                )}
                <div className="grid grid-cols-1 gap-2">
                  <button
                    onClick={() => {
                      void downloadOahuOfflinePack(11, 14);
                    }}
                    disabled={offlinePackState === 'downloading'}
                    className="bg-accent/15 border border-accent text-accent p-2 rounded text-[9px] font-black uppercase disabled:opacity-50"
                  >
                    Download Standard z11-z14
                  </button>
                  <button
                    onClick={() => {
                      void downloadOahuOfflinePack(15, 16);
                    }}
                    disabled={offlinePackState === 'downloading'}
                    className="bg-amber-500/15 border border-amber-400 text-amber-300 p-2 rounded text-[9px] font-black uppercase disabled:opacity-50"
                  >
                    Download High Detail z15-z16
                  </button>
                  <button
                    onClick={() => {
                      void clearOahuOfflinePack();
                    }}
                    className="bg-bg/30 border border-border text-text-dim p-2 rounded text-[9px] font-black uppercase hover:text-white"
                  >
                    Clear Cache
                  </button>
                </div>
          ...
          <div className="mt-auto pt-6 border-t border-border">
            ...
            <div className="mt-4 grid grid-cols-2 gap-2">
              <button
                onClick={exportAllLocalData}
                className="bg-bg/40 border border-border p-2 rounded text-[9px] font-black uppercase hover:border-accent hover:text-accent transition-all flex items-center justify-center gap-1"
              >
              ...
              <button
                ...
                className="bg-bg/40 border border-border p-2 rounded text-[9px] font-black uppercase hover:border-accent hover:text-accent transition-all flex items-center justify-center gap-1"
              >
                <FileJson size={10} /> Import Data
              </button>
              <input
                ref={importDataInputRef}
                type="file"
                accept="application/json"
                onChange={importAllLocalData}
                className="hidden"
              />
            </div>
          </div>
